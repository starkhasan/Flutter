...
import 'package:hello_flutter/providers/ExpandableCardProvider.dart';
...
class _ExpandableCardList extends State<ExpandableCardList> with TickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Container(
          child: ChangeNotifierProvider<ExpandableCardProvider>(
            create: (context) => ExpandableCardProvider(),
            child: FutureBuilder(
              future: future,
              builder: (context, snapshot) {
                if (ConnectionState.done == snapshot.connectionState) {
                  if (snapshot.data == '400' || snapshot.data == '500') {
                    ...
                    return Consumer<ExpandableCardProvider>(
                      builder: (context,expandProvider,child){
                        return ListView.builder(
                          key: Key('builder ${expandProvider.selected.toString()}'),
                          itemCount: response.length,
                          itemBuilder: (context, index) {
                            return Card(
                              child: ExpansionTile(
                                key: Key(index.toString()),
                                initiallyExpanded: expandProvider.selected == index,
                                onExpansionChanged: (value) => expandProvider.expandCard(index),
